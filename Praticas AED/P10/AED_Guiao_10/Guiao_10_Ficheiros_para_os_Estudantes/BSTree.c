...
// A: é a melhor porque na estrutura tao os nodes para baixo
// e para a aceder o próximo temos que tem o anterior logo é melhor apagar por último os nodes mais acima
...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
  ...
  if (heightLeft > heightRight){
    //o +1 serve para contar o nó atual
    return 1 + heightLeft;
  }
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  //ir o mais a esquerda possivel 
  struct _BSTreeNode* min = header->root;
  while (min->left != NULL) {
    min = min->left;
  }
  return min->item;
...
// A: it is because the items are inserter in crescent order

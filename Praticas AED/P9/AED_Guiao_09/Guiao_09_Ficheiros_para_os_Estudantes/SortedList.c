...
int ListSearch(List* l, const void* p) {
  ...
  while (i < l->size && l->compare(p, sp->item) > 0){
    i++;
    sp = sp->next;
  }
...
void ListMove(List* l, int newPos) {
  ...
  if (newPos == -1) {  // move outside
    ...
    if (l->currentPos == -1 || l->currentPos == l->size || newPos < l->currentPos) {
      l->current = l->head;
      l->currentPos = 0;
    }
...
void* ListRemoveCurrent(List* l) {
  ...
  if (l->currentPos == 0)
    ...
    struct _ListNode* sp = l->head;
    while (sp -> next != l->current){
      sp = sp->next;
    }
    sp->next = l->current->next;
